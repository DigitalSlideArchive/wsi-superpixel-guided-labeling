...
export default Vue.extend({
    ...
    watch: {
        selectedSuperpixel() {
            ...
            this.predictionCategories = _.pluck(this.selectedSuperpixel.predictionCategories);
        },
    ...
    methods: {
        ...
        filterSuperpixels(data) {
            ...
            if (this.filterBy.includes('reviewed')) {
                results.push(_.filter(data, (superpixel) => !_.isNull(superpixel.reviewCategory)));
            }
        ...
        applyBulkReview(newValue) {
            _.forEach(this.selectedReviewSuperpixels, (superpixel) => {
                ...
                const userData = labels.get('annotation').elements[0].user;
                // If no new value is provided user selection is correct
                userData.reviews[superpixel.index] = newValue || superpixel.selectedCategory;
                userData.reviewers[this.superpixel.index] = {
                    reviewer: store.currentUser,
                    date: new Date().toDateString()
                };
                store.reviewedSuperpixels += _.isNull(newValue) ? -1 : 1;
            });
            store.backboneParent.saveAnnotations(_.keys(store.annotationsByImageId));
            this.selectedReviewSuperpixels = [];
        },
        reviewInformation(superpixel) {
            superpixel = superpixel || this.selectedSuperpixel;
            if (!superpixel) {
                return [];
            }
            const labels = store.annotationsByImageId[superpixel.imageId].labels;
            const userData = labels.get('annotation').elements[0].user;
            return {
                value: userData.reviews[superpixel.index],
                reviewer: userData.reviewers[superpixel.index].reviewer || '',
                date: userData.reviewers[superpixel.index].date || ''
            };
        },
        isReviewed(superpixel) {
            superpixel = superpixel || this.selectedSuperpixel;
            const { value } = this.reviewInformation(superpixel);
          ...
          <div v-if="selectedSuperpixel">
            ...
            >
              <tbody>
                <tr>
                  <td>Selected</td>
                  <td>{{ selectedSuperpixel.labelCategories[reviewInformation().value].label }}</td>
                </tr>
                <tr>
                  <td>Reviewer</td>
                  <td>{{ reviewInformation().reviewer }}</td>
                </tr>
                <tr>
                  <td>Date</td>
                  <td>{{ reviewInformation().date }}</td>
                </tr>
              </tbody>
            </table>
            <h6 v-else>
          ...
          <div class="bulk-buttons">
            ...
            <div class="dropdown btn-group-three">
              ...
              >
                ...
                <li
                  v-for="category, index in predictionCategories"
                  :key="index"
                >
                  <div class="radio">
                    <label class="options">
          ...
          <div class="panel-content-cards">
            ...
            >
              <active-learning-review-card
                ...
                :selecting-superpixels="selectingSuperpixels"
                data-toggle="modal"
                data-target="#context"
                @click.native="selectedSuperpixel = superpixel"
              />
              <div
                v-if="isReviewed(superpixel)"
