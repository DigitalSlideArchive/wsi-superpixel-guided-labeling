    ...
    methods: {
        ...
        combineCategories(isMerge) {
            ...
            const newValues = store.categories.map((category) => {
                ...
                if (index === -1) {
                    return isMerge ? store.categoriesAndIndices.length + 1 : 0;
                }
                return index;
            });
            ...
            store.categoriesAndIndices.map((catAndIdxs) => {
                Object.keys(catAndIdxs.indices).forEach((imageId) => {
            ...
            store.categoriesAndIndices = store.categoriesAndIndices.filter((_, i) => !this.checkedCategories.includes(i + 1));

            // Update the exclusions list
            store.exclusions = _.without(store.exclusions, ...this.checkedCategories);
